...
Chart.js via CDN (gráfico de rosca e radar)
...
Deixar desativado. Mais simples e mais seguro para um site público; nenhum dado sai do navegador do visitante.
Colar uma chave própria direto em apiKey. Funciona, mas fica visível a qualquer pessoa que veja o código-fonte da página (é tudo client-side) — só recomendável com uma chave restrita por domínio e com cota baixa, nunca uma chave de produção.
Rotear as chamadas por um backend simples (uma function serverless, por exemplo) que guarda a chave no servidor e nunca a expõe ao navegador. É o padrão correto para produção, mas exige mais do que um único arquivo estático.
Problema conhecido
